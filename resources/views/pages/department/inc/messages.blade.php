...
<script>
    ...
    $(function() {
        ...
        @if ($message_room->type == 'single')
            getSingleRoomMessage('#messageRoom_{{ $message_room->id }}', {{ $message_room->id }}, '{{ ucwords($message_room->user->getName()) }}', '{{ ucwords($message_room->internship->title) }}', '{{ $message_room->user->avatar }}')
        @else
        ...
        if(socket_is_allowed) {
            ...
            socket.emit('user_connection', ['i' @foreach(auth()->user()->department->internships as $r) , {{ $r->id }} @endforeach]);
    ...
    function ajaxRequestUrl() {
        return "{{ getenv('APP_URL') }}/api/department/";
    }
        ...
        $.get(ajaxRequestUrl() + 'get-message/' + room_id, function(data) {
            ...
            if (data.length == 0) {
                ...
                for (let i = 0; i < data.length; i++) {
                    if (data[i].sender_type == 'Internship') {
            ...
            if(r == 'onfocusout'){
                ...
                if(room_data != null && room_data != 'occ' && socket_is_allowed){
                    socket.emit('delete_typing_private_message', {
                        receiver : room_data.user,
                        sender : room_data.internship,
                        receiverType: 'u',
                        room_id : room_id
                    })
                }
